...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		receipt, _, err := ApplyInstrumentedTransaction(p.config, p.bc, nil, gp, statedb, header, tx, usedGas, cfg, gethProcess)
		if err != nil {
...
func ApplyInstrumentedTransaction(config *params.ChainConfig, bc *BlockChain, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config, gethProcess *process.Process) (*types.Receipt, uint64, error) {
